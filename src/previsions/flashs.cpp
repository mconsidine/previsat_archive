...
int Flashs::CalculFlashs(int &nombre)
{
    ...
    while (it4.hasNext()) {
        ...
        if (!_resultatSat.isEmpty()) {
            _resultats.insert(sat.tle().norad(), _resultatSat);
        }
    }
 ...
 */
QPair<double, double> Flashs::CalculAngleMin(const QList<double> jjm, Satellite &satellite, Soleil &soleil)
{
    /* Declarations des variables locales */
    QList<double> ang;
...
void Flashs::CalculLimitesFlash(const double mgn0, const double dateMaxFlash, Satellite &satellite, Soleil &soleil, Date lim[])
{
    ...
    jjm.append(jj0 - PAS1);
    jjm.append(0.5 * (dateMaxFlash + jj0 - PAS1));
    jjm.append(dateMaxFlash);
...
void Flashs::LimiteFlash(const double mgn0, const QList<double> jjm, Satellite &satellite, Soleil &soleil, double limite[])
{
    ...
    for (int i=0; i<3; i++) {
        ...
        mag.append(MagnitudeFlash(ang[i], condEcl, satellite));
    }
