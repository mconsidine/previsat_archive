...
void General::AffichageDonneesSatellite(const Date &date)
{
    ...
    if ((delai >= -EPS_DATES) && (_dateEclipse->jourJulienUTC() < satellite.traceAuSol().last().jourJulienUTC)) {
        ...
        const QString cDelaiEcl = (delai >= (NB_JOUR_PAR_HEUR - EPS_DATES)) ?
                    delaiEcl.ToShortDate(DateFormat::FORMAT_COURT, DateSysteme::SYSTEME_24H).mid(11, 5)
                    .replace(":", tr("h", "hour").append(" ")).append(tr("min", "minute")) :
                    delaiEcl.ToShortDate(DateFormat::FORMAT_COURT, DateSysteme::SYSTEME_24H).mid(14, 5)
...
void General::SauveOngletGeneral(const QString &fichier)
{
    ...
    try {
        ...
        if (sw.open(QIODevice::WriteOnly | QIODevice::Text)) {
            ...
            chaine = tr("Azimut (N) : %1\t\tDéclinaison      : %2 \tIllumination : %3", "Azimuth from the North");
            flux << chaine.arg(_ui->azimutLune->text().trimmed()).arg(_ui->declinaisonLune->text()).arg(_ui->illuminationLune->text()) << Qt::endl;

            chaine = tr("Distance   : %1  \t\tConstellation    : %2 \t\t\tMagnitude    : %3");
            flux << chaine.arg(_ui->distanceLune->text()).arg(_ui->constellationLune->text()).arg(_ui->magnitudeLune->text()) << Qt::endl << Qt::endl;
